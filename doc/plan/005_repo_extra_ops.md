...
### 3.1 后端指令支持 (Rust)
- [ ] **新增指令 `get_git_log(path: String, count: i32)`**:
  - 获取指定数量的提交记录。
  - 返回字段：`id` (short hash), `author`, `time`, `message`。
- [ ] **新增指令 `open_in_folder(path: String)`**:
  - 调用 `tauri_plugin_opener` 实现跨平台打开文件夹。
- [ ] **新增指令 `open_in_terminal(path: String)`**:
  - 根据操作系统分发指令：
...
## 4. 验收标准
- [ ] 点击“提交记录”标签，能看到提交人、时间及注释。
- [ ] 点击“打开目录”，系统资源管理器成功弹出并定位到该仓库。
- [ ] 点击“终端”，成功打开终端且当前路径已 `cd` 到仓库根目录。
