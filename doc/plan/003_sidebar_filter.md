...
- [ ] **自动展开**: 当进行有效搜索时，应自动展开包含匹配项的文件夹。
- [ ] **无结果处理**: 搜索无结果时在侧边栏显示“无匹配结果”。

### 3.2 同步路线过滤 (Routes.vue)
- [ ] **同步过滤逻辑**: 在 `Routes.vue` 中实现类似的递归过滤逻辑，处理 `route_tree_data`。
- [ ] **路径搜索支持**: 支持对源仓库/目标仓库名称的模糊匹配。
